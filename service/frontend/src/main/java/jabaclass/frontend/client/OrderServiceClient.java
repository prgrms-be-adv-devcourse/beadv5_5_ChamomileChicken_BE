...
public class OrderServiceClient {
    ...
    private OrderSummaryDto mapToOrderSummary(Map m) {
        ...
        if (m.get("quantity") != null) {
            dto.setQuantity((int) m.get("quantity"));
        }
        if (m.get("totalAmount") != null) {
            dto.setTotalAmount(new BigDecimal(m.get("totalAmount").toString()));
        }
        if (m.get("status") != null) {
            dto.setStatus(m.get("status").toString());
        }
        return dto;
    }
    ...
    private CreateOrderResponse mapToCreateOrderResponse(Map m) {
        ...
        dto.setProductScheduleId(m.get("productScheduleId") != null ? UUID.fromString(m.get("productScheduleId").toString()) : null);
        dto.setProductUserId(m.get("productUserId") != null ? UUID.fromString(m.get("productUserId").toString()) : null);
        if (m.get("quantity") != null) {
            dto.setQuantity((int) m.get("quantity"));
        }
        if (m.get("totalAmount") != null) {
        ...
        if (m.get("depositAmount") != null) {
            dto.setDepositAmount(new BigDecimal(m.get("depositAmount").toString()));
        }
        if (m.get("paymentAmount") != null) {
            dto.setPaymentAmount(new BigDecimal(m.get("paymentAmount").toString()));
        }
        if (m.get("status") != null) {
            dto.setStatus(m.get("status").toString());
        }
