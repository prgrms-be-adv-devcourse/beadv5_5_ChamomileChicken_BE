...
> MSA 환경에서 병목을 구간별로 식별하기 위해 Prometheus/Grafana 기반 정량 지표와 AOP/TraceId 기반 요청 추적을 함께 도입했다. 이를 통해 요청 → 스레드풀 → 외부 API → 커넥션 풀 → 쿼리 → 응답의 6개 구간을 기준으로 성능 병목을 분석할 수 있는 기반을 마련했다.
